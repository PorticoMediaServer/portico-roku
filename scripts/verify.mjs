...
assert(/PorticoPlaybackBeginTerminal\(controller, active, "stopped"/.test(playbackStopSource) && /generation = PorticoPlaybackBoundedSeconds\(playback\.sessionGeneration, 0\)/.test(playbackTaskSource), 'Playback stop must use the session actor generation and canonical terminal envelope');
assert(/PorticoSecureRegistryCommit\("playback-mutation", mutation\)/.test(playbackTaskSource) && /PorticoPlaybackTerminalAcknowledgementMatches/.test(playbackTaskSource) && /if pending\.kind = "terminal" then method = "DELETE"/.test(playbackTaskSource), 'Playback terminal authority must persist one exact DELETE body and clear only after its durable acknowledgement');
assert(!/completed/.test(playbackProgressSource), 'Ordinary progress PATCH must never carry terminal completion state');
