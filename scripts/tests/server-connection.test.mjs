...
assert.match(routeRecovery, /PorticoServerConnectionRaceStoredRoutes\(controller, source\)/);
assert.match(routeRecovery, /PorticoServerConnectionAdoptStoredRoute\(controller, source, route\)/);
const routeRace = task.match(/function PorticoServerConnectionRaceStoredRoutes[\s\S]*?\nend function/)?.[0] ?? '';
assert.match(routeRace, /nowMs >= 500 or cachedFinished/);
assert.match(routeRace, /PorticoServerConnectionStartHostedRace/);
assert.match(routeRace, /PorticoServerConnectionConsumeRouteProbeEvent\(cached, message\)/);
assert.match(routeRace, /PorticoServerConnectionConsumeRouteProbeEvent\(hosted\.probes, message\)/);
assert.match(task, /startBaseMs \+ 150/);
assert.match(routeRace, /hosted\.incompatible[\s\S]*PorticoServerConnectionFail\(controller, "hosted-incompatible"/);
const consumeProbe = task.match(/function PorticoServerConnectionConsumeRouteProbeEvent[\s\S]*?\nend function/)?.[0] ?? '';
